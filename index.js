...
exports.etfInfo = async (req, res) => {
  ...
  try {
    ...
    const segment = await page.evaluate((segmentSelector) => {
      return document.querySelector(segmentSelector)
        ? document.querySelector(segmentSelector).textContent
        : "";
    }, segmentSelector);
    ...
    const aum = await page.evaluate((aumLabelText) => {
      ...
      for (var i = 0; i < labels.length; ++i) {
        if (
          ...
          labels[i].textContent.trim() == aumLabelText
        ) {
          aumString = labels[i].nextElementSibling
            ? labels[i].nextElementSibling.textContent // "$123.45M"
            : "";
          break;
        }
      }
    ...
    const er = await page.evaluate((erLabelText) => {
      ...
      for (var i = 0; i < labels.length; ++i) {
        if (
          labels[i] &&
          labels[i].textContent &&
          labels[i].textContent.trim() == erLabelText
        ) {
          return labels[i].nextElementSibling
            ? labels[i].nextElementSibling.textContent // "0.65%"
            : "";
        }
      }
    }, erLabelText);
    ...
    const perf = await page.evaluate(() => {
      return {
        YTD: document.querySelectorAll(".perfYtd")[1]
          ? document.querySelectorAll(".perfYtd")[1].textContent
          : "",
        "1M": document.querySelectorAll(".perf1Mo")[1]
          ? document.querySelectorAll(".perf1Mo")[1].textContent
          : "",
        "3M": document.querySelectorAll(".perf3Mo")[1]
          ? document.querySelectorAll(".perf3Mo")[1].textContent
          : "",
        "1Y": document.querySelectorAll(".perf1Yr")[1]
          ? document.querySelectorAll(".perf1Yr")[1].textContent
          : "",
        "3Y": document.querySelectorAll(".perf3YrAnnualized")[1]
          ? document.querySelectorAll(".perf3YrAnnualized")[1].textContent
          : "",
        "5Y": document.querySelectorAll(".perf5YrAnnualized")[1]
          ? document.querySelectorAll(".perf5YrAnnualized")[1].textContent
          : "",
        "10Y": document.querySelectorAll(".perf10YrAnnualized")[1]
          ? document.querySelectorAll(".perf10YrAnnualized")[1].textContent
          : "",
      };
    });
    ...
    const dividend = await page.evaluate((dividendLabelText) => {
      ...
      for (var i = 0; i < labels.length; ++i) {
        if (
          labels[i] &&
          labels[i].textContent &&
          labels[i].textContent.trim() == dividendLabelText
        ) {
          return labels[i].nextElementSibling
            ? labels[i].nextElementSibling.textContent // "0.65%"
            : "";
    ...
    const inception = await page.evaluate((inceptionLabelText) => {
      ...
      for (var i = 0; i < labels.length; ++i) {
        if (
          labels[i] &&
          labels[i].textContent &&
          labels[i].textContent.trim() == inceptionLabelText
        ) {
          return labels[i].nextElementSibling
            ? labels[i].nextElementSibling.textContent // "12/20/93"
            : "";
        }
      }
    }, inceptionLabelText);
    ...
    const flow5d = strToNum(
      $(".net-fund-flow.5-day").text().trim().split("\n")[1]
    );
    const flow1m = strToNum(
      $(".net-fund-flow.1-month").text().trim().split("\n")[1]
    );
